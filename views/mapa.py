...
# Ferramentas Avançadas do Folium (Removido o MiniMap para performance)
from folium.plugins import HeatMap, MarkerCluster, Fullscreen, Draw, LocateControl, MeasureControl
try:
...
def render_mapa_otimizado(df_notas_mapa, df_eq_mapa_view, criticos_tuple, caminho_camada_temp, mapa_lat, mapa_lon, estilo_mapa, visao_cores, camada_fundo, raio_km, tipo_rota, f_lev):
    ...
    if not df_eq_mapa_view.empty:
        ...
        if 'LATITUDE' in df_eq.columns and 'LONGITUDE' in df_eq.columns:
            ...
            for _, row in df_eq.dropna(subset=['Lat', 'Lon']).iterrows():
                ...
                if raio_km > 0:
                    folium.Circle(
                        location=[row['Lat'], row['Lon']], radius=raio_km * 1000, 
                        color='#10b981', fill=True, fill_opacity=0.1, weight=1, tooltip=f"Cobertura Logística: {raio_km}km"
                    ).add_to(fg_equipes)
    ...
    if not df_notas_mapa.empty:
        ...
        if mask_miss.any() and 'MUNICIPIO' in df_ob.columns:
            dict_ma_lat = {'SAO LUIS': -2.53, 'IMPERATRIZ': -5.52, 'BALSAS': -7.53, 'PINHEIRO': -2.52} 
        ...
        # --- TRAVA DE SEGURANÇA PARA ROTEIRIZAÇÃO ---
        if tipo_rota != "Nenhuma":
            if f_lev == "TODOS" and len(dict_bases) > 1:
                st.warning("⚠️ **Alerta de Performance:** Para evitar lentidão severa, a Roteirização só é traçada quando você filtra **1 Técnico específico** no menu lateral.")
            else:
    ...
    map_data = st_folium(mapa, use_container_width=True, height=800, returned_objects=["all_drawings"])
...
def view_mapa():
    ...
    with st.sidebar:
        ...
        estilo_mapa = st.radio("Visualização das Obras:", ["Agrupamentos (Clusters)", "Pinos Individuais", "Calor (Heatmap)"]) # Deixei cluster padrão pra velocidade
    ...
    with st.spinner("Construindo renderização geográfica de alta precisão..."):
        map_state, df_plotado = render_mapa_otimizado(df_m, df_e, tuple(levantadores_criticos), camada_p, mapa_lat, mapa_lon, estilo_mapa, visao_cores, camada_fundo, raio_km, tipo_rota, f_lev)
    ...
    if map_state and map_state.get("all_drawings"):
        ...
        for d in drawings:
            ...
            if geom.get("type") in ["Polygon", "Rectangle"]:
                ...
                polygons.append([(c[1], c[0]) for c in coords]) 
