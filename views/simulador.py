...
import numpy as np
from database import load_core_data, get_base_levantadores, save_base_levantadores, SEM_LEVANTADOR, STATUS_PRODUTIVIDADE
...
def view_simulador():
    ...
    # Extração de Métricas Chaves (Base Geral de Notas)
    total_municipios = df_equipes_territorio['Município'].nunique() if 'Município' in df_equipes_territorio.columns else 217
    
    # Métrica de Cobertura (Olhando para a base oficial de municípios/levantadores)
    df_eq_clean = df_equipes_territorio.copy()
    col_lev = 'Levantador' if 'Levantador' in df_eq_clean.columns else 'LEVANTADOR'
    
    # Conta os municípios onde o "Dono" oficial é o SEM LEVANTADOR
    municipios_livres = df_eq_clean[df_eq_clean[col_lev].astype(str).str.strip().str.upper() == 'SEM LEVANTADOR'].shape[0]
    municipios_cobertos = total_municipios - municipios_livres
    cobertura_mun = (municipios_cobertos / total_municipios * 100) if total_municipios > 0 else 0
    ...
    c2.markdown(kpi_card("Total Obras (Geral)", f"{len(df_notas)}", "🏗️", "#8b5cf6"), unsafe_allow_html=True) 
    c3.markdown(kpi_card("Municípios Livres", f"{municipios_livres}", "⚠️", "#f59e0b"), unsafe_allow_html=True) 
    c4.markdown(kpi_card("Cobertura Territorial", f"{cobertura_mun:.1f}%", "🎯", "#10b981"), unsafe_allow_html=True) 
    st.markdown("<br><br>", unsafe_allow_html=True)

    # =====================================================================
    # 3. TABELA UNIFICADA INTERATIVA (Cenário de Alocação)
    # =====================================================================
    ...
    for reg in df_reg['REGIONAL']:
        if reg not in st.session_state.sim_atuais:
            st.session_state.sim_atuais[reg] = 0
            
    df_reg['Equipes Atuais'] = df_reg['REGIONAL'].map(st.session_state.sim_atuais).fillna(0).astype(int)
    
    # Coluna 3: Gap Atual (Contagem de Municípios SEM LEVANTADOR por Regional da Planilha)
    ...
    mun_livres_reg.columns = ['REGIONAL', 'Gap Atual (Munc)']
    mun_livres_reg['REGIONAL'] = mun_livres_reg['REGIONAL'].astype(str).str.upper()
    ...
    # Coluna 4: Novos Levantadores (Editável)
    if 'sim_novos' not in st.session_state:
        st.session_state.sim_novos = {reg: 0 for reg in df_reg['REGIONAL']}
        
    for reg in df_reg['REGIONAL']:
        if reg not in st.session_state.sim_novos:
            st.session_state.sim_novos[reg] = 0
    ...
    df_display.rename(columns={'Total_Obras': 'Total Obras'}, inplace=True)
    ...
    edited_df = st.data_editor(
        ...
        column_config={
            ...
            "Total Obras": st.column_config.NumberColumn("Total Obras (Geral)", disabled=True),
    ...
    # =====================================================================
    # 4. GESTOR OFICIAL DE MUNICÍPIOS E LEVANTADORES (Planilha ao Vivo)
    # =====================================================================
    ...
    # Prepara as opções de Dropdown (Garante que SEM LEVANTADOR seja opção válida)
